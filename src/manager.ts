import {
    AfterPointcut, AfterStaticPointcut, AroundPointcut, AroundStaticPointcut,
    BeforePointcut, BeforeStaticPointcut, GetterPointcut,
    InstanceMethodPointcut,
    InstancePropertyPointcut,
    Pointcut, SetterPointcut, StaticGetterPointcut,
    StaticMethodPointcut,
    StaticPropertyPointcut, StaticSetterPointcut
} from './annotations';
import { Ctor, getAllPropertyMetadata } from './metadata-utils';
...
export abstract class AbstractJoinpointContext<CONTEXT = any, P extends Pointcut<any> = Pointcut<any>> {
    constructor(protected readonly context: CONTEXT, protected readonly property: string, protected readonly pointcut: P) {}
 ...
 */
export class JoinpointContext<
    CONTEXT = any,
    P extends keyof CONTEXT = never,
    ARGS extends any[] = ArgumentTypes<CONTEXT[P]>,
    RESULT = ReturnType<CONTEXT[P]>
> extends AbstractJoinpointContext<CONTEXT, InstanceMethodPointcut<any> | StaticMethodPointcut<any>> {
...
export abstract class PropertyJoinpointContext<CONTEXT> extends AbstractJoinpointContext<
    CONTEXT,
    InstancePropertyPointcut<any> | StaticPropertyPointcut<any>
> {}

/**
 * Joinpoint context for getter advices
 */
export class GetterJoinpointContext<
    CONTEXT = any,
    P extends keyof CONTEXT = never,
    T = CONTEXT[P]
> extends PropertyJoinpointContext<CONTEXT> {
 ...
 */
export class SetterJoinpointContext<
    CONTEXT = any,
    P extends keyof CONTEXT = never,
    T = CONTEXT[P]
> extends PropertyJoinpointContext<CONTEXT> {
...
export class AopManager {
    ...
    install(aspects: any[]) {
        for (const aspect of aspects) {
            ...
            const annotationsMap = getAllPropertyMetadata(type);

            for (const [adviceName, annotations] of annotationsMap) {
    ...
    protected installMethodAdvices(aspect: any, adviceName: string | symbol, annotation: InstanceMethodPointcut<any>) {
        const pointcuts = findPointcuts(annotation.cls, annotation.property);
        for (const pointcut of pointcuts) {
            if (annotation instanceof BeforePointcut) {
                createJoinpoint(
                    function (this: any, target: Function, ...args: any[]) {
                    ...
                    pointcut,
                    annotation.cls.prototype
                );
            }
            if (annotation instanceof AfterPointcut) {
                createJoinpoint(
                    function (this: any, target: Function, ...args: any[]) {
                        const context = new JoinpointContext(args, this, pointcut, annotation);
            ...
            if (annotation instanceof AroundPointcut) {
                createJoinpoint(
                    function (this: any, target: Function, ...args: any[]) {
    ...
    protected installStaticMethodAdvices(aspect: any, adviceName: string | symbol, annotation: StaticMethodPointcut<any>) {
        const pointcuts = findStaticPointcuts(annotation.cls, annotation.property);
        for (const pointcut of pointcuts) {
            if (annotation instanceof BeforeStaticPointcut) {
                createJoinpoint(
                    function (this: any, target: Function, ...args: any[]) {
                        const context = new JoinpointContext(args, this, pointcut, annotation);
                        aspect[adviceName](context);
            ...
            if (annotation instanceof AfterStaticPointcut) {
                createJoinpoint(
                    function (this: any, target: Function, ...args: any[]) {
                        const context = new JoinpointContext(args, this, pointcut, annotation);
                        context.setResult(target.apply(this, args));
            ...
            if (annotation instanceof AroundStaticPointcut) {
                createJoinpoint(
                    function (this: any, target: Function, ...args: any[]) {
                        const context = new JoinpointContext(args, this, pointcut, annotation, target);
                        return aspect[adviceName](context);
    ...
    protected installPropertyAdvices<T extends {}>(aspect: any, adviceName: string | symbol, annotation: InstancePropertyPointcut<T>) {
        const pointcuts = toPointcuts<T>(annotation.property);
        for (const pointcut of pointcuts) {
            if (annotation instanceof GetterPointcut) {
                createPropertyJoinpoint(
                    function (this: any, { getter }: { getter: Function }) {
            ...
            if (annotation instanceof SetterPointcut) {
                createPropertyJoinpoint(
                    function (this: any, { setter }: { setter: Function }, value: any) {
    ...
    protected installStaticPropertyAdvices<T extends {}>(aspect: any, adviceName: string | symbol, annotation: StaticPropertyPointcut<T>) {
        const pointcuts = toPointcuts<T>(annotation.property);
        for (const pointcut of pointcuts) {
            if (annotation instanceof StaticGetterPointcut) {
                createPropertyJoinpoint(
                    function (this: any, { getter }: { getter: Function }) {
                        const context = new GetterJoinpointContext(this, pointcut, annotation, getter);
                        return aspect[adviceName](context);
            ...
            if (annotation instanceof StaticSetterPointcut) {
                createPropertyJoinpoint(
                    function (this: any, { setter }: { setter: Function }, value: any) {
                        const context = new SetterJoinpointContext(this, pointcut, annotation, setter, value);
                        aspect[adviceName](context);
 ...
 * @hidden
 */
 export function findPointcuts<T extends {}>(cls : Ctor<T>, selector : keyof T | string[] | RegExp) : string[] {
 ...
 * @hidden
 */
export function findStaticPointcuts<T extends Ctor<any>>(cls : T, selector : keyof T | string[] | RegExp) : string[] {
